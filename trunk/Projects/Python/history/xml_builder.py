...
def build(filename):
	...
	for line in f_read:
		...
		if(edit == 0): #Yet more laziness. Start of time period
			...
			text = line[sep+2:len(line)-1] #+2 for : and whitespace
			write_event(f_write, time_num, text)
		
	f_write.write("\t\t</events>\n")
	f_write.write("\t</period>\n")
...
def get_time(string): #Very clunky, I'm sure there is a nice library to do this for me somewhere
	...
	if("years ago" in string or "bc" in string or "BC" in string):
		bonus *= -1
