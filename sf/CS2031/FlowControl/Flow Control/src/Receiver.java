	...
	public class sendingThread implements Runnable {
		public void run() {
			...
			while(true){
				byte[] temp = {1};
				if(ack){
					System.out.println("Acknowledged. Lets hope it dosent get lost to noise");
					ack = false;
					out.send(temp);
				}
			}
		...
		public void run() {
			...
			while(!complete) {
				...
				if (frame!=null) {
					...
					offset+= frame.length;
					System.out.println(frame[frame.length-1]+":"+frame[frame.length-2]);
					if ((frame[frame.length-1]== (byte)255) && (frame[frame.length-2]==(byte)255)){
						complete= true;
					}
					ack = true;
				}
