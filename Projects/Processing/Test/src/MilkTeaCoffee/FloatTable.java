...
class FloatTable {
	...
	FloatTable(String filename, PApplet p) {
		this.p = p;
		String[] rows = p.loadStrings(filename);

		String[] columns = p.split(rows[0], p.TAB);
		columnNames = p.subset(columns, 1); // upper-left corner ignored
		...
		for (int i = 1; i < rows.length; i++) {
			...
			String[] pieces = p.split(rows[i], p.TAB);
			scrubQuotes(pieces);

			// copy row title
			rowNames[rowCount] = pieces[0];
			// copy data into the table starting at pieces[1]
			data[rowCount] = p.parseFloat(p.subset(pieces, 1));

			// increment the number of valid rows found so far
			rowCount++;
		}
		// resize the 'data' array as necessary
		data = (float[][]) p.subset(data, 0, rowCount);
	}
